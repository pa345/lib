 ...
 * stage1c.c
 *
 * 1. Read TIEGCM 3D magnetic field grids for some time interval
...
/* fill magfield grid with Br,Bt,Bp */
int
...
/*
main_proc()
  Compute SH decomposition of each radial shell of TIEGCM 3D magnetic
field values; store SH coefficients to disk
...
main_proc(const char * prefix, size_t * tidx, time_t * tsteps, const tiegcm3d_data * data, magfield_workspace * w)
{
  ...
  for (i = 0; i < data->nt; ++i)
    {
      fprintf(stderr, "main_proc: filling magnetic field grid for timestep %zu/%zu\n", *tidx + 1, ttot);
...
main(int argc, char *argv[])
{
  ...
  char *file_prefix = PCA3D_STAGE1C_SH_PREFIX;
  ...
  while (optind < argc)
    {
      ...
      data = tiegcm3d_read_B(argv[optind], NULL);
      if (!data)
        {
  ...
  /* write magfield parameter data and TIEGCM data to disk */
  {
    ...
    fprintf(stderr, "main: writing meta-data file %s...", PCA3D_STAGE1C_DATA);
    pca3d_write_data(PCA3D_STAGE1C_DATA, &d);
    fprintf(stderr, "done\n");
  }
