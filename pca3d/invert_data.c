...
static int invert_data_print_satellite(const char *dir_prefix, const gsl_vector *wts_spatial,
                                       const size_t nsource, const magdata * mptr, size_t * index);
static int invert_data_print_observatory(const char *dir_prefix, const gsl_vector *wts_spatial,
                                         const magdata * mptr, size_t * index);
static int invert_data_print_observatory_SV(const char *dir_prefix, const gsl_vector *wts_spatial,
                                            const magdata * mptr, size_t * index);

/*
invert_data_alloc()
  Allocate invert_data_workspace
...
invert_data_alloc(const size_t nsources, const invert_data_parameters *params)
{
  invert_data_workspace *w;

  w = calloc(1, sizeof(invert_data_workspace));
  if (!w)
    return 0;
...
void
invert_data_free(invert_data_workspace *w)
{
  if (w->t0)
    free(w->t0);
...
size_t
invert_data_filter_time(const double tmin, const double tmax,
                        invert_data_workspace *w)
{
  size_t cnt = 0;
  size_t i, j;
...
/*
invert_data_filter_observatory()
...
size_t
invert_data_filter_observatory(invert_data_workspace *w)
{
...
int
invert_data_print(const char *dir_prefix, const gsl_vector *wts_spatial,
                  const invert_data_workspace *w)
{
  int s = 0;
  size_t i;
  size_t idx = 0;
  ...
  for (i = 0; i < w->nsources; ++i)
    {
      ...
      if (mptr->global_flags & MAGDATA_GLOBFLG_SATELLITE)
        invert_data_print_satellite(dir_prefix, wts_spatial, i, mptr, &idx);
      else if (mptr->global_flags & MAGDATA_GLOBFLG_OBSERVATORY)
        invert_data_print_observatory(dir_prefix, wts_spatial, mptr, &idx);
      else if (mptr->global_flags & MAGDATA_GLOBFLG_OBSERVATORY_SV)
        invert_data_print_observatory_SV(dir_prefix, wts_spatial, mptr, &idx);
    }
...
magdata *
invert_data_ptr(const size_t idx, const invert_data_workspace *w)
{
  if (idx >= w->nsources)
    {
      fprintf(stderr, "invert_data_ptr: invalid index: %zu\n", idx);
      return 0;
    }
...
static int
invert_data_print_satellite(const char *dir_prefix, const gsl_vector *wts_spatial,
                            const size_t nsource, const magdata * mptr, size_t * index)
{
  int s = 0;
  ...
  for (j = 0; j < n; ++j)
    {
      if (fp[j] == NULL)
        {
          fprintf(stderr, "invert_data_print_satellite: fp[%zu] is NULL\n", j);
          return -1;
        }
    }
...
static int
invert_data_print_observatory(const char *dir_prefix, const gsl_vector *wts_spatial,
                              const magdata * mptr, size_t * index)
{
  int s = 0;
  ...
  for (j = 0; j < n; ++j)
    {
      if (fp[j] == NULL)
        {
          fprintf(stderr, "invert_data_print_observatory: fp[%zu] is NULL\n", j);
          return -1;
        }
    }
...
/*
invert_data_print_observatory_SV()
...
static int
invert_data_print_observatory_SV(const char *dir_prefix, const gsl_vector *wts_spatial,
                                 const magdata * mptr, size_t * index)
{
  int s = 0;
  const char *fmtstr = "%ld %8.4f %6.3f %10.4f\n";
