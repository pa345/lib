 ...
 * stage2b_ind.c
 *
 * 1. Read in spherical harmonic time series gnm from stage1 matrix file (nnm-by-nt)
 * 2. Subtract mean from each gnm(t) time series
 * 2. Compute SVD of centered gnm matrix
...
/* define to subtract mean from gnm(t) time series prior to SVD */
...
Inputs: ut       - UT hour in [0,23]
        G        - matrix of gnm(t), nnm-by-nt
        ut_array - array of UT values, size nt
*/

int
calc_pca(const size_t ut, const gsl_matrix * G, const double *ut_array)
{
  int s = 0;
  const size_t nnm = G->size1;
  const size_t nt_all = G->size2;
  size_t i;
  size_t nt = 0; /* number of timestamps for this UT */
  gsl_matrix *A = gsl_matrix_alloc(nnm, nt_all);
  gsl_vector *mu = gsl_vector_calloc(nnm); /* mean values of gnm(t) for this UT */
  gsl_matrix_view m;
  struct timeval tv0, tv1;

  fprintf(stderr, "calc_pca: finding gnm coefficients corresponding to %zu UT...", ut);
  ...
  for (i = 0; i < nt_all; ++i)
    {
      ...
      if (ut == uti)
        {
          gsl_vector_const_view v = gsl_matrix_const_column(G, i);
          gsl_vector_view w = gsl_matrix_column(A, nt);

          /* copy set of gnm coefficients for this UT hour into the A matrix */
...
#if 0
#if SUBTRACT_MEAN
  fprintf(stderr, "calc_pca: subtracting mean from each %zu UT gnm time series...", ut);
...

  {
    ...
    sprintf(sval_txt_file, "%s_%02zuUT.txt", PCA_STAGE2B_SVAL_TXT_IND, ut);
    sprintf(sval_file, "%s_%02zuUT.dat", PCA_STAGE2B_SVAL_IND, ut);
    sprintf(U_file, "%s_%02zuUT.dat", PCA_STAGE2B_U_IND, ut);
    sprintf(V_file, "%s_%02zuUT.dat", PCA_STAGE2B_V_IND, ut);
    sprintf(mu_file, "%s_%02zuUT.dat", PCA_STAGE2B_MU_IND, ut);
...
main(int argc, char *argv[])
{
  ...
  pca_read_data(PCA_STAGE1_DATA_IND, &nmax, &mmax, &nt, ut_array);
  assert(nt == G->size2);
  ...
  /* taper spherical harmonic coefficients to eliminate ringing effect from TIEGCM */
  {
    ...
    gsl_vector_view x = gsl_matrix_column(G, 0);

    fprintf(stderr, "main: tapering gnm coefficients...");
    taper_knm(20, G, green_p);
    fprintf(stderr, "done\n");
...
#if SUBTRACT_MEAN
  {
    ...
    fprintf(stderr, "main: subtracting mean from each gnm time series...");
    subtract_mean(G, mu);
    fprintf(stderr, "done\n");
  ...
  for (ut = 0; ut < 24; ++ut)
    {
      calc_pca(ut, G, ut_array);
    }
  exit(1);
#endif

  /* compute 1/sqrt(nt) G for SVD computation */
  gsl_matrix_scale(G, 1.0 / sqrt((double) nt));

  {
    const char *sval_txt_file = "sval_time_ind.txt";
    const char *sval_file = PCA_STAGE2B_SVAL_IND;
    const char *U_file = PCA_STAGE2B_U_IND;
    const char *V_file = PCA_STAGE2B_V_IND;
    gsl_vector *S = gsl_vector_alloc(GSL_MIN(nnm, nt));
    gsl_matrix *U = gsl_matrix_alloc(nnm, nnm);
    gsl_matrix *V = gsl_matrix_alloc(nt, nt);
    int status;
    ...
    fprintf(stderr, "main: performing SVD of G matrix (%zu-by-%zu)...", nnm, nt);
    gettimeofday(&tv0, NULL);
    status = lapack_svd(G, S, U, V);
    gettimeofday(&tv1, NULL);
    fprintf(stderr, "done (%g seconds, status = %d)\n",
            time_diff(tv0, tv1), status);
