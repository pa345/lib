 ...
 * stage1_ind.c
 *
 * 1. Read tiegcm data file(s) for induced field from Alexey/Alexander
 *    The B grid in these files contains B_ext + B_ind, so B_ext must
 *    be subtracted
 * 2. Read stage1_knm.dat file previously generated on the same dataset by stage1.c
 * 3. Subtract B_ext from B_tot to give B_ind
 * 4. Invert B_ind at each timestep for SH coefficients g_{nm}(t_k)
 * 5. Store g_{nm}(t) SH coefficients in a nnm-by-nt matrix:
 *
 *      X_{ij} = g_i(t_j) where i = shidx(n,m)
 * 6. X matrix is output to a binary file
 *
 * ./stage1_ind [-o binary_output_matrix_file] tiegcm1.nc tiegcm2.nc ...
 */
                  ...
                  green_workspace *green_int, gsl_matrix *A)
{
  ...
  if (p_ext > 0)
    {
      X_ext = gsl_vector_alloc(p_ext);
      Y_ext = gsl_vector_alloc(p_ext);
      Z_ext = gsl_vector_alloc(p_ext);
    }
...
int
subtract_B_ext(tiegcm_data *data)
{
  ...
  gsl_matrix *K = pca_read_matrix(PCA_STAGE1_KNM); /* read knm matrix from previously computed B_ext grid */
  double *counter = malloc(max_threads * sizeof(double));
  size_t k;

  assert(data->nt == K->size2);
  ...
  for (k = 0; k < data->nt; ++k)
    {
      int thread_id = omp_get_thread_num();
      size_t ilon, ilat;
      gsl_vector_const_view knm = gsl_matrix_const_column(K, k);
      ...
      for (ilon = 0; ilon < data->nlon; ++ilon)
        {
          ...
          for (ilat = 0; ilat < data->nlat; ++ilat)
            {
              size_t idx = TIEGCM_BIDX(k, ilat, ilon, data);
              double theta = M_PI / 2.0 - data->glat[ilat] * M_PI / 180.0;
              double B_ext[3];
              ...
              gsl_blas_ddot(Z[thread_id], &knm.vector, &B_ext[2]);

              /* subtract B_ext from B_tot */
              data->Bx[idx] -= B_ext[0] * 1.0e-9;
              data->By[idx] -= B_ext[1] * 1.0e-9;
              data->Bz[idx] -= B_ext[2] * 1.0e-9;
            }
        }

      counter[thread_id] += 1.0;

      if (thread_id == 0)
        {
          gsl_vector_view v = gsl_vector_view_array(counter, max_threads);
          double sum = gsl_blas_dasum(&v.vector);
          double progress = sum / (double) data->nt;
...
main_proc(const char *filename, const char *outfile_mat, const char *outdir_mat, tiegcm_data *data)
{
...
#if 1/*XXX*/
  fprintf(stderr, "main_proc: subtracting B_ext grids...");
  subtract_B_ext(data);
  fprintf(stderr, "done\n");
#endif
  ...
  /* print spectrum of internal coefficients at time t_0 */
  {
    const size_t k = 0;
  ...
  fprintf(fp, "# Field %zu: g(1,0) (nT)\n", k++);
  fprintf(fp, "# Field %zu: g(1,1) (nT)\n", k++);
  fprintf(fp, "# Field %zu: g(2,0) (nT)\n", k++);
  fprintf(fp, "# Field %zu: g(2,1) (nT)\n", k++);
  fprintf(fp, "# Field %zu: g(2,2) (nT)\n", k++);
  ...
  fprintf(stderr, "main_proc: writing misc data to %s...", PCA_STAGE1_DATA_IND);
  pca_write_data(PCA_STAGE1_DATA_IND, nmax_int, mmax_int, data);
  fprintf(stderr, "done\n");
  ...
  fprintf(stderr, "main_proc: wrote gnm coefficients to %s\n", filename);
...
main(int argc, char *argv[])
{
  ...
  struct timeval tv0, tv1;
  char *outfile = "gnm.txt";
  char *outdir_mat = NULL;
  char *outfile_mat = PCA_STAGE1_GNM;
