...
main_proc(const char *filename, const char *outfile_mat, const char *outdir_mat, tiegcm_data *data)
{
...
#if TAPER_COEFFS
    taper_knm(30, X_taper, green_ext_p);
#endif
  }
  ...
  /* print residuals for a given timestamp */
  {
    ...
    rnorm = print_residuals(res_file, k, &AA.matrix, &x.vector, data, green_ext_p);
      ...
      for (N = 1; N <= 2; ++N)
        {
          ...
          for (m = 0; m <= M; ++m)
            {
              size_t cidx = green_nmidx(N, m, green_ext_p);
      ...
      for (k = 0; k < data->nt; ++k)
        {
          ...
          print_matlab(outdir_mat, k, &x.vector, green_ext_p);
          fprintf(stderr, "done\n");
